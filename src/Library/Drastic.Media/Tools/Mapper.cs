// <copyright file="Mapper.cs" company="Drastic Actions">
...
namespace Drastic.Media.Core.Infrastructure
{
    internal static class Mapper
    {
        internal static PodcastShowItem Map(Uri podcastUri, Rss rss, string imagesStorage = "")
        {
            ...
            var episodes = rss.Channel.Item.Where(item => item.Enclosure != null).Select(n => Map(n, imageUrl)).ToList();
            var copyright = rss.Channel.Copyright ?? string.Empty;
            var show = new PodcastShowItem(podcastUri, author, rss.Channel.Description, email, language, rss.Channel.Title, link, image, copyright, pubDate);
            episodes.ToList().ForEach(episode => show.Episodes.Add(episode));

            return show;
        }

        private static PodcastEpisodeItem Map(Item item, string image)
        {
            var imageUrl = (image ?? item.Image2?.Href) ?? throw new ArgumentNullException("Image.Url");
            ...
            var episode = new PodcastEpisodeItem(description, duration, @explicit == "yes", pubDate, item.Title, url, imageUrl);
            return episode;
        }
